...
class Model_progress extends CI_Model
{
    ...
    function project_location_fetch_data($item_code, $project_code)
    {
        ...
        $query = $this->db->query("SELECT DISTINCT progressmaster.LocationCode, progressmaster.ProjectCode, locationsubmaster.Status, userproject.UserName FROM progressmaster INNER JOIN locationsubmaster ON progressmaster.LocationCode=locationsubmaster.LocationCode INNER JOIN userproject ON progressmaster.ProjectCode=userproject.ProjectCode where userproject.status='checked' && progressmaster.ProjectCode='$project_code' && progressmaster.PhotoItem='$item_code'");
        return $query;
    ...
    function project_image_fetch_data($item_code, $project_code, $location_code)
    {
        $query = $this->db->query("SELECT * FROM progressmaster LEFT JOIN userproject ON progressmaster.ProjectCode=userproject.ProjectCode where userproject.status='checked' && progressmaster.ProjectCode='$project_code' && progressmaster.PhotoItem='$item_code' && progressmaster.LocationCode='$location_code'");
        return $query;
